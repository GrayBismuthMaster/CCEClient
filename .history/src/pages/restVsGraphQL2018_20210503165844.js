...
const RestVsGraphQL = () => {
    const [datosGraphQL, setDatosGraphql] = useState("")
    ...
    const {data, loading, error}= useQuery(FRECUENCIAUSOINTERNET2018);
        useEffect(()=>{    
            setDatosGraphql(data.obtenerFrecuenciaUsoInternet2018.tamanio)
    },[])
        if(data!==null)
        {
    ...
    let startRest = new Date().getTime();
        ...
        const tamanioDatos = ()=>{
            console.log("datos" +datosGraphQL)
            return <text>{datosGraphQL}</text>
        }
        const tamanioDatos2 = ()=>{
            
            return <text>{datosGraphQL.obtenerFrecuenciaUsoInternet2018.obtenerFrecuenciaUsoInternet2018.tamanio}</text>
        }
    return (
         <>
        <div style={{
            ...
            <div style={{   display:"flex",
                ...
                <ul style={{
                    ...
                    <li>
                        Tamaño de los datos : {tamanioDatos()+"bytes"}
                    </li>
                </ul>
            </div>
            <div style={{   display:"flex",
                    ...
                    <li>
                        Tamaño de los datos : {`${()=>tamanioDatos2()} bytes`}
                    </li>
                </ul>
            </div>
